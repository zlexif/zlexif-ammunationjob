...
RegisterNetEvent('zlexif-ammunation:client:Eat', function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"burger"}) -- Animation Eating
    QBCore.Functions.Progressbar("eat_something", Language.Progressbars.Eating..QBCore.Shared.Items[itemName].label.."..", 6000, false, true, {
        ...
        TriggerServerEvent("zlexif-ammunation:Server:RemoveItem", itemName, 1)
        ...
        if not QBCore.Shared.Items[itemName].thirst and not QBCore.Shared.Items[itemName].hunger then
            TriggerServerEvent("QBCore:Server:SetMetaData", "hunger", QBCore.Functions.GetPlayerData().metadata["hunger"] + math.random(40,80))
        end
...
RegisterNetEvent('zlexif-ammunation:client:Drink', function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"coffee"}) -- Animation Drinking
    QBCore.Functions.Progressbar("drink_something", Language.Progressbars.Drinking..QBCore.Shared.Items[itemName].label.."..", 6000, false, true, {
        disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true,
    }, {}, {}, {}, function()
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[itemName], "remove", 1)
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent('zlexif-ammunation:client:DrinkAlcahol', function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"whiskey"}) -- Animation Drinking
    QBCore.Functions.Progressbar("drink_something", Language.Progressbars.Drinking..QBCore.Shared.Items[itemName].label.."..", 7000, false, true, {
        disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true,
    }, {}, {}, {}, function()
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[itemName], "remove", 1)
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        ...
        if QBCore.Shared.Items[itemName].hunger then TriggerServerEvent("QBCore:Server:SetMetaData", "hunger", QBCore.Functions.GetPlayerData().metadata["hunger"] + QBCore.Shared.Items[itemName].hunger) end
        if not QBCore.Shared.Items[itemName].thirst and not QBCore.Shared.Items[itemName].hunger then
            TriggerServerEvent("QBCore:Server:SetMetaData", "thirst", QBCore.Functions.GetPlayerData().metadata["thirst"] + math.random(25,45))
        end
